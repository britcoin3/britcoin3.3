...
static const char *strDNSSeed[][2] = {
    {"node01.britcoin.xyz", "node01.britcoin.xyz"},
    {"node02.britcoin.xyz", "node02.britcoin.xyz"},
};
// hidden service seeds
static const char *strMainNetOnionSeed[][1] = {
    {"w3guguvt2hqjollp.onion"},
    {"hvteqd7jwfffjsay.onion"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BritCoin is probably already running."), addrBind.ToString().c_str());
        else
