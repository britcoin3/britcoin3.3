...
static const int64_t V3_DRIFT_PARAMS_TIME = 1477828800; // V3 timedrift switch, 30 Oct 2016 12 noon GMT (see presstab interview)
...
static const int INVESTOR_COIN_MINT_HEIGHT = 1056500;  // mint new coins on Wed 2nd Nov 2016
...
inline int64_t PastDrift(int64_t nTime)   {
    if (nTime < V2_DRIFT_PARAMS_TIME){
        ...
        return nTime - 5 * 60; // New time drift maximum -- 5 mins
        }
}
...
public:
    ...
    {
        // britcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // britcoin: get transaction coin age
...
public:
    ...
    // britcoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // britcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // britcoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // britcoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // britcoin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // britcoin: block index flags
    enum  
    {
